...
\begin{itemize}
  \item manipuler le dataset \textit{employees} dans PostgreSQL (exploration, contrôles, jointures) ;
...
\subsection{Enregistrements \textit{actuels} (to\_date = 9999-01-01)}
...
\subsection{Exploration du schéma (colonnes)}
...
\subsubsection{Accès à la vue matérialisée}
...
\subsubsection{Accès à la jointure directe}
...
La vue matérialisée réduit le temps d’accès pour compter les lignes (\textbf{532.128 ms} contre \textbf{1460.525 ms}) car elle évite de recalculer la jointure à chaque requête : les données jointes sont déjà matérialisées et peuvent être relues plus directement.
...
\subsection{5.1 — Jointure employees \texorpdfstring{$\leftrightarrow$}{<->} titles (1 \texttt{\$lookup})}
...
\subsection{5.2 — Jointure employees \texorpdfstring{$\leftrightarrow$}{<->} titles \texorpdfstring{$\leftrightarrow$}{<->} salaries (2 \texttt{\$lookup})}
...
\subsection{5.3 — Temps d’exécution de la jointure MongoDB}
...
\subsection{5.4 — Dénormalisation avec \texttt{\$project} (suppression des doublons)}
...
\subsection{5.5 — Sauvegarde dans une nouvelle collection (\texttt{\$merge})}
...
Temps observé :
\begin{itemize}
  \item \textbf{creation\_employees\_denormalises 4:47.119 (m:ss.mmm)}
  \item soit \textbf{287.119 s} (environ)
\end{itemize}

\subsection{5.6 — Délai d’accès aux informations après dénormalisation}
...
Accès à un employé dénormalisé par \_id (find + explain) & 0 ms \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \textbf{On veut éviter le coût récurrent des jointures} : ici, la jointure MongoDB sur tout le dataset a coûté \textbf{56.4 s}, alors qu’après dénormalisation l’accès par \texttt{\_id} est immédiat (explain : \textbf{0 ms} observé).
